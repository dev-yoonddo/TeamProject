	...
	<section>
		...
		<h4 style="font-weight: bold; color: #646464;">문의목록</h4><br>
			...
			<%
				if(pageNumber != 1) {
			%>
				<button type="button" id="prev" class="btn-black" onclick="location.href='customerPage.jsp?pageNumber=<%=pageNumber - 1%>'"><span>이전</span></button>
				
			<% 
				} if(bdDAO.nextPage(pageNumber + 1)){ 
			%>
				<button type="button" id="next" class="btn-black" onclick="location.href='customerPage.jsp?pageNumber=<%=pageNumber + 1%>'"><span>다음</span></button>
